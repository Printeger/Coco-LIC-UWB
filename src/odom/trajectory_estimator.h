...
#include <odom/factor/analytic_diff/uwb_factor.h>
...
class TrajectoryEstimator {
 ...
 public:
  ...
  void AddPhotometricMeasurementAutoDiffNURBS(
      const double &prev_pixel_intensity,
      const Eigen::Vector3d &visual_map_point, const cv::Mat &cur_img,
      int64_t cur_img_timestamp, const SO3d &S_VtoI,
      const Eigen::Vector3d &p_VinI, const Eigen::Matrix3d &K,
  ...
  void AddPoseMeasurementAutoDiffNURBS(const PoseData &pose_data,
                                       double pos_weight, double rot_weight);

  void AddPoseMeasurementAnalyticDiffNURBS(const PoseData &pose_data,
                                           double pos_weight,
                                           double rot_weight);

  void AddRelativePoseMeasurementAnalyticDiffNURBS(const PoseData &pose_data,
                                                   double pos_weight,
                                                   double rot_weight);
  ...
  ceres::Solver::Summary PreSolve(int max_iterations = 50,
                                  bool progress = false, int num_threads = -1);
  ...
  void AddControlPointsNURBS(size_t start_idx, std::vector<double *> &vec,
                             bool addPosKnot = false);
 ...
 private:
  ...
  void PrepareMarginalizationInfo(ResidualType r_type,
                                  ...
                                  ceres::LossFunction *loss_function,
                                  std::vector<double *> &parameter_blocks,
                                  std::vector<int> &drop_set_w_ctrl_point,
                                  bool is_lidar_inertial);
