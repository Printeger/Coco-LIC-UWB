...
void TrajectoryEstimator::AddControlPointsNURBS(size_t start_idx,
...
void TrajectoryEstimator::AddControlPointsNURBS(size_t start_idx1,
                                                size_t start_idx2,
                                                std::vector<double *> &vec,
                                                bool addPosKnot) {
...
void TrajectoryEstimator::AddIMUMeasurementAnalyticNURBS(
  ...
  // trajectory_->maxTimeNsNURBS() << std::endl;
  ...
  ceres::CostFunction *cost_function = new analytic_derivative::IMUFactorNURBS(
      time_ns, imu_data, gw, info_vec, trajectory_->knts, su, blending_matrix,
...
void TrajectoryEstimator::AddLoamMeasurementAnalyticNURBS(
  ...
  ceres::CostFunction *cost_function =
      new Functor(time_ns, pc, su, blending_matrix, cumulative_blending_matrix,
...
void TrajectoryEstimator::AddPnPMeasurementAnalyticNURBS(
    const Eigen::Vector3d &visual_map_point, const Eigen::Vector2d &pixel_obs,
    int64_t cur_img_timestamp, const SO3d &S_VtoI,
    const Eigen::Vector3d &p_VinI, const Eigen::Matrix3d &K,
    double img_weight) {
  ...
  ceres::CostFunction *cost_function =
      new Functor(time_ns, su, blending_matrix, cumulative_blending_matrix,
                  visual_map_point, pixel_obs, S_VtoI, p_VinI, K, img_weight);
  ...
  ceres::LossFunction *loss_function = NULL;
  loss_function =
      new ceres::CauchyLoss(10.0);  // adopt from vins-mono  //[default: 10.0]
  problem_->AddResidualBlock(cost_function, loss_function, vec);
}

void TrajectoryEstimator::AddPhotometricMeasurementAnalyticNURBS(
    // const double &prev_pixel_intensity,
    int patch_size_half, int scale, int level, float *patch,
    const Eigen::Vector3d &visual_map_point, const cv::Mat &cur_img,
    int64_t cur_img_timestamp, const SO3d &S_VtoI,
    const Eigen::Vector3d &p_VinI, const Eigen::Matrix3d &K,
    double img_weight) {
  int64_t time_ns = cur_img_timestamp;
  ...
  ceres::CostFunction *cost_function = new Functor(
      patch_size_half, scale, level, time_ns, su, blending_matrix,
      cumulative_blending_matrix,
      // prev_pixel_intensity,
      patch, visual_map_point, cur_img, S_VtoI, p_VinI, K, img_weight);
...
void TrajectoryEstimator::AddPoseMeasurementAnalyticDiffNURBS(
    const PoseData &pose_data, double pos_weight, double rot_weight) {
  ...
  info_vec << rot_weight, rot_weight, rot_weight, pos_weight, pos_weight,
      pos_weight;

  ceres::CostFunction *cost_function =
      new analytic_derivative::IMUPoseFactorNURBS(
          time_ns, pose_data, info_vec, trajectory_->knts, su, blending_matrix,
          cumulative_blending_matrix);
...
void TrajectoryEstimator::AddRelativePoseMeasurementAnalyticDiffNURBS(
    const PoseData &pose_data, double pos_weight, double rot_weight) {
  int64_t time_ns = pose_data.timestamp;
  std::pair<int, double> su;  // i u
  trajectory_->GetIdxT(time_ns, su);
  ...
  ceres::CostFunction *cost_function =
      new analytic_derivative::IMURelativePoseFactorNURBS(
...
void TrajectoryEstimator::AddRalativeLoamFeatureAnalytic(
  ...
  if (!MeasuredTimeToNs(LiDARSensor, pc.t_map, time_map_ns)) return;
  if (!MeasuredTimeToNs(LiDARSensor, pc.t_point, time_point_ns)) return;
  SplineMeta<SplineOrder> spline_meta;
...
void TrajectoryEstimator::AddImageFeatureOnePoseAnalytic(
    ...
    bool marg_this_fearure) {
  int64_t time_j_ns;
  if (!MeasuredTimeToNs(CameraSensor, t_j, time_j_ns)) return;
  SplineMeta<SplineOrder> spline_meta;
...
void TrajectoryEstimator::AddEpipolarFactorAnalytic(
    ...
    bool marg_this_fearure) {
  int64_t time_i_ns;
  if (!MeasuredTimeToNs(CameraSensor, t_i, time_i_ns)) return;
  SplineMeta<SplineOrder> spline_meta;
