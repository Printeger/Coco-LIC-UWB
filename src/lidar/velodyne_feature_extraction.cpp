...
void VelodyneFeatureExtraction::LidarHandler(
  ...
  if (raw_cloud->isOrganized()) {
    std::cout << "[OrganizedCloudToRangeImage]\n";
    OrganizedCloudToRangeImage(raw_cloud, range_mat, p_full_cloud);
  } else {
    std::cout << "[RTCloudToRangeImage]\n";
...
bool VelodyneFeatureExtraction::ParsePointCloud(
  ...
  if (!has_checked) {
    ...
    has_time_field =
        CheckMsgFields(*lidar_msg, "time");  // float s -> Velodyne: lvi、lio
    has_t_field =
        CheckMsgFields(*lidar_msg, "t");  // uint32_t ns -> Ouster: viral
    has_timestamp_field =
        CheckMsgFields(*lidar_msg, "timestamp");  // float s -> Hesai-PandarQT

    check_field_passed = has_ring_field &&
                         (has_time_field || has_t_field || has_timestamp_field);

    std::cout << "has_ring_field: " << has_ring_field
              << " has_time_field: " << has_time_field
              << " has_t_field: " << has_t_field
              << " has_timestamp_field: " << has_timestamp_field
              << " check_field_passed: " << check_field_passed << std::endl;

    if (!has_ring_field)
      LOG(WARNING) << "[ParsePointCloud] input cloud NOT has [ring] field";

    if (!has_time_field && !has_t_field && !has_timestamp_field)
      LOG(WARNING) << "[ParsePointCloud] input cloud NOT has [time] or [t] or "
                      "[timestamp] field";
  }
  /// convert cloud
  if (check_field_passed) {
    if (has_time_field) {
      ...
      std::cout << "=====tmp_out_cloud size: " << tmp_out_cloud->points.size()
                << std::endl;
      RTPointCloudTmp2RTPointCloud(tmp_out_cloud, out_cloud);
    } else if (has_t_field) {
      std::cout << "[has_t_field] " << std::endl;
      // for ouster 64
      ...
      std::cout << "[has_timestamp_field] " << std::endl;
      // for hesai pandar
...
void VelodyneFeatureExtraction::OrganizedCloudToRangeImage(
  ...
  for (size_t column_id = 0; column_id < cur_cloud->width; ++column_id) {
    for (size_t row_id = 0; row_id < cur_cloud->height; ++row_id) {
      ...
      if (row_id < 0 || row_id >= size_t(n_scan)) continue;
      if (column_id < 0 || column_id >= size_t(horizon_scan)) continue;
...
void VelodyneFeatureExtraction::RTCloudToRangeImage(
    const RTPointCloud::Ptr cur_cloud, cv::Mat &dist_image,
    RTPointCloud::Ptr &corresponding_cloud) const {
  std::cout << "cur_cloud->points size: " << cur_cloud->points.size()
            << std::endl;
  ...
  for (const RTPoint &p : cur_cloud->points) {
    if (!std::isfinite(p.x) || !std::isfinite(p.y) || !std::isfinite(p.z)) {
      std::cout << "p issue: " << p.x << " " << p.y << " " << p.z << std::endl;
      continue;
    }
    int row_id = p.ring;
    if (row_id < 0 || row_id >= n_scan) {
      std::cout << "row_id issue: " << row_id << std::endl;
      continue;
    }
    ...
    if (column_id < 0 || column_id >= horizon_scan) {
      std::cout << "column_id issue: " << column_id << std::endl;
      continue;
    }

    float range = pcl::PointNorm<RTPoint>(p);
    if (range < fea_param_.min_distance || range > fea_param_.max_distance) {
      std::cout << "range issue: " << range << std::endl;
      continue;
    }

    if (dist_image.at<float>(row_id, column_id) != FLT_MAX) {
      std::cout << "dist_image issue: "
                << dist_image.at<float>(row_id, column_id) << std::endl;
      continue;
    }
    dist_image.at<float>(row_id, column_id) = range;
...
void VelodyneFeatureExtraction::CloudExtraction() {
  ...
  for (int i = 0; i < n_scan; i++) {
    ...
    for (int j = 0; j < horizon_scan; j++) {
      // std::cout << "range_mat.at<float>(i, j): " << range_mat.at<float>(i, j)
      //           << std::endl;
